...
-(void)setImageUrl:(NSString *)imageUrl duration:(NSInteger)duration skipType:(SkipType)skipType options:(XHWebImageOptions)options completed:(XHWebImageCompletionBlock)completedBlock click:(clickBlock)click
{
    ...
    [_adImgView xh_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:@"5.5.png"] options:options completed:completedBlock];
}
...
- (void)AFNetworkReachabilityStatus{
    __weak XHLaunchAd *weakSelf = self;
