...
+ (NSString *)documentPathWithSubDirectoryName:(NSString *)subdir{
    NSString *docPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *subDirectory = [docPath stringByAppendingPathComponent:subdir];
    BOOL isDirectory = NO;
    NSFileManager *fm = [NSFileManager defaultManager];
    if (![fm fileExistsAtPath:subDirectory isDirectory:&isDirectory]) {
        NSError *error = nil;
        if (![fm createDirectoryAtPath:subDirectory withIntermediateDirectories:YES attributes:nil error:&error]) {
            [NSException raise:@"Failed creating directory" format:@"[%@], %@", subDirectory, error];
        }
    }
